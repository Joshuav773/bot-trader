...
export default function ForexPage() {
  ...
  const [newsSentimentThreshold, setNewsSentimentThreshold] = useState(0.05);
  ...
  useEffect(() => {
    async function loadPairs() {
      try {
        ...
        if (!token) return;
        const res = await fetch(`${API_URL}/forex/pairs`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.ok) {
          setAvailablePairs(await res.json());
        }
      } catch (e) {
        // Ignore
      }
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/forex/confluence/backtest`, {
        method: "POST",
        headers: {
  ...
  async function optimize() {
    ...
    try {
      const token = localStorage.getItem("bt_jwt");
      if (!token) throw new Error("Missing token. Please login.");
      const res = await fetch(`${API_URL}/forex/confluence/optimize`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      // Auto-fill best parameters if single pair
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar currentPage="/forex" />
      <main className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold bg-gradient-to-r from-primary-600 to-primary-800 bg-clip-text text-transparent">Forex Confluence Strategy</h1>
          <p className="text-gray-500 text-sm mt-1">
            Optimized confluence strategy for major forex pairs (EURUSD, GBPUSD, USDJPY, etc.)
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg border border-gray-200/50 p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Forex Pair</label>
                <select
                  value={pair}
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-2 h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
                  />
                  <span className="text-sm text-gray-700">Optimize All Major Pairs (Learning Agent)</span>
                </label>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {optimizing ? "Learning..." : optimizeAll ? "Optimize All" : "Optimize"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            {result && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Backtest Result</h3>
            ...
            {optimizeResult && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Learning Agent Result</h3>
                {optimizeResult.aggregate_best_parameters ? (
                  <div className="space-y-4">
                    <div>
                      <p className="text-sm text-gray-600 mb-1">Aggregate Best Parameters</p>
                      <div className="bg-gray-50 rounded-md p-3">
                        <pre className="text-xs text-gray-700 overflow-auto">
                          {JSON.stringify(optimizeResult.aggregate_best_parameters, null, 2)}
                        </pre>
                      </div>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 mb-1">Results by Pair</p>
                      <div className="bg-gray-50 rounded-md p-3 max-h-96 overflow-auto">
                        <pre className="text-xs text-gray-700">
                          {JSON.stringify(optimizeResult.results_by_pair, null, 2)}
                        </pre>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {optimizeResult.top_5_results && (
                      <div>
                        <p className="text-sm text-gray-600 mb-2">Top 5 Results</p>
                        <div className="bg-gray-50 rounded-md p-3 max-h-64 overflow-auto">
                          <pre className="text-xs text-gray-700">
                            {JSON.stringify(optimizeResult.top_5_results, null, 2)}
                          </pre>
                        </div>
                      </div>
                    )}
